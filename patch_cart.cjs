...
code = code.replace(
  /<div className="flex flex-col lg:flex-row gap-12">[\s\S]*?<div className="flex-1 bg-white shadow-sm p-4 md:p-6">/m,
...
code = code.replace(
  /className="absolute top-0 right-0 md:hidden p-2 text-neutral-400 hover:text-red-500"/g,
...
// 4. Thumbnail
code = code.replace(
  /className="w-24 aspect-\[3\/4\] shrink-0 bg-white border border-gray-200 block"/g,
  'className="w-28 md:w-32 aspect-[3/4] shrink-0 bg-[#f8f9fa] border border-gray-100/80 rounded-md overflow-hidden block group-hover:border-[#2874f0]/20 transition-colors"'
);

// 5. Title, Color, Size
code = code.replace(
  /<div className="flex flex-col pt-2">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/,
...
// We need to apply it globally for map
code = code.replace(
  /<div className="flex flex-col pt-2">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>/g,
  `<div className="flex flex-col pt-1">
                      <Link to={\`/product/\${item.product.id}\`} className="text-sm md:text-base font-semibold text-[#1a1a1a] hover:text-[#2874f0] transition-colors mb-2 pr-8 md:pr-0 line-clamp-2 leading-snug">
                        {item.product.name}
                      </Link>
...
// 6. Price (desktop)
code = code.replace(
  /className="hidden md:block col-span-2 text-center font-sans text-sm text-\[\#212121\]"/g,
  'className="hidden md:block col-span-2 text-center text-[15px] font-bold text-[#212121]"'
);

// 7. Quantity Control
const quantityRegex = /<div className="w-full md:w-auto md:col-span-2 flex justify-start md:justify-center">\s*<div className="flex items-center border border-gray-200 h-10 w-28 bg-white">[\s\S]*?<button[\s\S]*?<\/button>\s*<\/div>\s*<\/div>/g;

const quantityReplacement = `<div className="w-full md:w-auto md:col-span-2 flex justify-start md:justify-center">
...
code = code.replace(quantityRegex, quantityReplacement);

// 8. Remove button desktop
const removeDesktopRegex = /<div className="hidden md:flex col-span-2 justify-end items-center gap-4 font-sans text-sm text-\[\#212121\]">[\s\S]*?<\/button>\s*<\/div>/g;
const removeDesktopReplacement = `<div className="hidden md:flex col-span-2 justify-end items-center gap-4 text-[15px] font-bold text-[#212121]">
...
code = code.replace(removeDesktopRegex, removeDesktopReplacement);

// 9. Order Summary Card
const summaryRegex = /{[\s\S]*?\/\* Order Summary \*\/[\s\S]*?<div className="w-full lg:w-\[350px\] shrink-0">[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*<\/div>\s*\);\s*};/m;
...
// Replace the Empty Cart
code = code.replace(
  'className="min-h-[70vh] pt-32 pb-20 bg-[#f1f3f6] flex flex-col items-center justify-center px-4 text-center"',
  'className="min-h-[70vh] pt-32 pb-20 bg-[#f1f3f6] flex flex-col items-center justify-center px-4 text-center"'
);
code = code.replace(
